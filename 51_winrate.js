	...
	grapher.draw = function(node) {
		...
		for (let n = 0; n < eval_list.length; n++) {
			...
			if (e !== null) {
				...
				if (last_x !== null) {
					this.add_line(last_x, last_y, x, y, interp ? "#666666" : "white", 2, interp);
				}
		...
		if (depth !== 0 && depth !== eval_list.length - 1) {
			this.add_line(width * depth / (eval_list.length - 1), 0, width * depth / (eval_list.length - 1), height, "#6cccee", 1, true);
		}
	};
